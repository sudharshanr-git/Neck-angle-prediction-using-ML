#here the degree calculation is not done after cropping but naive method. input data is from cropped img
import cv2
...
while cap.isOpened():
    ...
    if results.detections:
        for detection in results.detections:
            ...
            nose_point = mp.solutions.face_detection.get_key_point(detection, mp.solutions.face_detection.FaceKeyPoint.NOSE_TIP)
            h, w, _ = frame.shape
            # .x value is normalized between 0 and 1 wrt width
            left_eye = (int(left_eye.x * w), int(left_eye.y * h))
            right_eye = (int(right_eye.x * w), int(right_eye.y * h))
            x, y = int(nose_point.x * frame.shape[1]), int(nose_point.y * frame.shape[0])
            cv2.circle(frame, (x, y), 5, (0, 255, 0), -1)
            mid=(int((left_eye[0]+right_eye[0])/2),int((left_eye[1]+right_eye[1])/2))
            cv2.circle(frame, mid, 5, (0, 255, 0), -1)
            cv2.line(frame, mid, (x, y), (255, 0, 0), 2)
            if mid[0]!=x:
                slope=(mid[1]-y)/(mid[0]-x)
            ...
            #print("right:",right_eye,"left:",left_eye)
            #print("slope:",slope)
    ...
    cv2.putText(mirrored_frame, f"{degree:.2f}", (30, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
